...
- [ ] 12.5. 商务账号管理模块（工厂老板）
  ...
  - [x] 12.5.4 实现商务账号管理前端页面
    ...
    - 实现删除商务账号功能（确认对话框）
    - 创建商务账号详情页面（工作统计）
    - _Requirements: 12.1, 12.2, 12.5, 12.6, 12.7, 12.8, 12.10_

- [x] 12.6. Checkpoint - 商务账号管理验证
  - 确保商务账号列表显示正常
  - 确保配额检查正确（达到上限时拒绝创建）
  - 确保启用/禁用功能正常
  - 确保删除功能正常（数据保留）
