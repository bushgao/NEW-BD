...
class WeChatMainWnd(WeChatSubWnd):
    # 支持微信 3.9 (WeChatMainWndForPC) 和微信 4.x (Qt51514QWindowIcon)
    _ui_cls_name: str = 'WeChatMainWndForPC'
    _ui_cls_name_4x: str = 'Qt51514QWindowIcon'  # 微信 4.x 使用 Qt 框架
    _ui_name: str = '微信'
    ...
    def __init__(self, nickname: str = None, hwnd: int = None):
        ...
        if hwnd:
            ...
            # 同时搜索 3.9 和 4.x 的窗口类名
            wxs = [i for i in GetAllWindows() if i[1] in (self._ui_cls_name, self._ui_cls_name_4x)]
            if len(wxs) == 0:
            ...
            # 优先尝试微信 4.x（Qt51514QWindowIcon）
            hwnd = FindWindow(classname=self._ui_cls_name_4x, name=self._ui_name)
            if not hwnd:
                # 回退到微信 3.9（WeChatMainWndForPC）
                hwnd = FindWindow(classname=self._ui_cls_name)
            if not hwnd:
                raise Exception(f'未找到微信窗口，请确保微信已登录')
            self._setup_ui(hwnd)
