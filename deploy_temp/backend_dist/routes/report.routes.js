...
const trendService = __importStar(require("../services/trend.service"));
 ...
 */
router.get('/staff-performance', (0, auth_middleware_1.requireRoles)('BRAND', 'PLATFORM_ADMIN', 'BUSINESS'), [
 ...
 */
router.get('/dashboard/trends', (0, auth_middleware_1.requireRolesOrIndependent)('BRAND', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('period').isIn(['week', 'month', 'quarter']).withMessage('周期参数无效'),
    (0, express_validator_1.query)('dataType').isIn(['gmv', 'cost', 'roi']).withMessage('数据类型参数无效'),
], validateRequest, async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
 ...
 */
router.get('/dashboard', (0, auth_middleware_1.requireRolesOrIndependent)('BRAND', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('period').optional().isIn(['week', 'month']).withMessage('周期参数无效'),
], validateRequest, async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
 ...
 * 获取商务人员个人看板数据
 */
router.get('/my-dashboard', (0, auth_middleware_1.requireRoles)('BUSINESS', 'BRAND'), [
    (0, express_validator_1.query)('period').optional().isIn(['week', 'month']).withMessage('周期参数无效'),
], validateRequest, async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
 ...
 * Requirements: 6.5
 */
router.get('/export/staff-performance', (0, auth_middleware_1.requireRolesOrIndependent)('BRAND', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('startDate').optional().isISO8601().withMessage('开始日期格式无效'),
    (0, express_validator_1.query)('endDate').optional().isISO8601().withMessage('结束日期格式无效'),
], validateRequest, async (req, res, next) => {
    try {
 ...
 * 导出ROI报表（Excel）
 */
router.get('/export/roi', (0, auth_middleware_1.requireRolesOrIndependent)('BRAND', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('groupBy').isIn(['influencer', 'sample', 'staff', 'month']).withMessage('分组参数无效'),
    (0, express_validator_1.query)('startDate').optional().isISO8601().withMessage('开始日期格式无效'),
    (0, express_validator_1.query)('endDate').optional().isISO8601().withMessage('结束日期格式无效'),
], validateRequest, async (req, res, next) => {
    try {
 ...
 * 导出合作记录（Excel）
 */
router.get('/export/collaborations', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('startDate').optional().isISO8601().withMessage('开始日期格式无效'),
    (0, express_validator_1.query)('endDate').optional().isISO8601().withMessage('结束日期格式无效'),
], validateRequest, async (req, res, next) => {
    try {
 ...
 * 获取 ROI 分析数据
 */
router.get('/dashboard/roi-analysis', (0, auth_middleware_1.requireRoles)('BRAND', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
            res.status(400).json({
 ...
 * 获取管道漏斗数据
 */
router.get('/dashboard/pipeline-funnel', (0, auth_middleware_1.requireRoles)('BRAND', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
            res.status(400).json({
 ...
 * 获取商务对比分析数据
 */
router.get('/staff/comparison', (0, auth_middleware_1.requireRoles)('BRAND', 'PLATFORM_ADMIN'), [
    (0, express_validator_1.query)('staffIds').isString().withMessage('商务ID列表参数无效'),
], validateRequest, async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
        ...
        // 解析商务ID列表（逗号分隔）
        const staffIdsStr = req.query.staffIds;
 ...
 * Requirements: FR-1.3
 */
router.get('/dashboard/daily-summary', (0, auth_middleware_1.requireRoles)('BRAND', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        if (!brandId) {
            res.status(400).json({
 ...
 */
router.get('/staff/:staffId/quality-score', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), (0, permission_middleware_1.checkStaffDataAccess)(), async (req, res, next) => {
    try {
 ...
 * @permission dataVisibility.viewOthersPerformance - 如果查看其他商务的数据，需要此权限
 */
router.get('/staff/:staffId/calendar', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), (0, permission_middleware_1.checkStaffDataAccess)(), async (req, res, next) => {
    try {
        const { staffId } = req.params;
 ...
 * Requirements: FR-1.3
 */
router.get('/dashboard/alerts', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
 ...
 * Requirements: FR-1.3
 */
router.put('/dashboard/alerts/:alertId/read', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
 ...
 * Requirements: FR-1.3
 */
router.put('/dashboard/alerts/read-all', (0, auth_middleware_1.requireRoles)('BRAND', 'BUSINESS', 'PLATFORM_ADMIN'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        const userId = req.user.userId;
        if (!brandId) {
 ...
 * Requirements: FR-2.4
 */
router.get('/my-dashboard/today-todos', (0, auth_middleware_1.requireRoles)('BUSINESS', 'BRAND'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        const staffId = req.user.userId;
        if (!brandId) {
 ...
 * Requirements: FR-2.4
 */
router.get('/my-dashboard/work-stats', (0, auth_middleware_1.requireRoles)('BUSINESS', 'BRAND'), async (req, res, next) => {
    try {
        const brandId = req.user.brandId;
        const staffId = req.user.userId;
