...
const dashboardLayoutValidation = [
    ...
    (0, express_validator_1.body)('layout.cards')
        .optional()
        .isArray()
 ...
 * @access Private
 */
router.get('/dashboard-layout', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
 ...
 * @access Private
 */
router.post('/dashboard-layout', auth_middleware_1.authenticate, dashboardLayoutValidation, handleValidationErrors, async (req, res, next) => {
    try {
        const userId = req.user.userId;
 ...
 * @access Private
 */
router.get('/saved-filters', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const user = await prisma_1.prisma.user.findUnique({
            where: { id: userId },
 ...
 * @access Private
 */
router.post('/saved-filters', auth_middleware_1.authenticate, [
    (0, express_validator_1.body)('name').trim().notEmpty().withMessage('筛选名称不能为空'),
    (0, express_validator_1.body)('filter').isObject().withMessage('筛选条件必须是对象'),
 ...
 * @access Private
 */
router.delete('/saved-filters/:filterId', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const userId = req.user.userId;
 ...
 * @access Private
 */
router.put('/saved-filters/:filterId/favorite', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { filterId } = req.params;
        const user = await prisma_1.prisma.user.findUnique({
