...
async function listTemplates() {
    return prisma_1.default.notificationTemplate.findMany({
...
async function seedDefaultTemplates() {
    for (const template of DEFAULT_TEMPLATES) {
        ...
        if (!existing) {
            await prisma_1.default.notificationTemplate.create({
                data: template, // 绕过Prisma的JSON类型检查
            });
 ...
 * 根据模板创建通知（支持变量替换）
 */
async function createNotificationFromTemplate(userId, templateType, variables = {}) {
