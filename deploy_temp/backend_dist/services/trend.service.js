 ...
 * 获取趋势数据
 */
async function getTrendData(brandId, period, dataType) {
    const now = new Date();
    // 计算日期范围
 ...
 * 计算日期范围
 */
function calculateDateRanges(now, period) {
    let days;
    let currentStart;
    let currentEnd;
    let previousStart;
 ...
 * 获取指定周期的数据
 */
async function fetchPeriodData(brandId, startDate, endDate, days, dataType) {
    // 获取该周期内的所有合作结果
