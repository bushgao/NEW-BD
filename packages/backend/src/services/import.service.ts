...
import { createBadRequestError, createQuotaExceededError } from '../middleware/errorHandler';
 ...
 */
export function parseFile(buffer: Buffer): { headers: string[]; rows: any[][] } {
  const workbook = XLSX.read(buffer, { type: 'buffer' });
  
  // Get first sheet
  const sheetName = workbook.SheetNames[0];
  if (!sheetName) {
    throw createBadRequestError('文件中没有找到工作表');
  }
  ...
  if (jsonData.length < 2) {
    throw createBadRequestError('文件中没有数据行');
  }
 ...
 */
function getValue(row: any[], headers: string[], field: string | undefined): string | undefined {
 ...
 */
function validateInfluencerRow(
  row: any[],
  headers: string[],
  mapping: InfluencerFieldMapping
): { data: InfluencerPreviewData | null; errors: string[] } {
  const errors: string[] = [];
...
export async function previewInfluencerImport(
  ...
  if (!headers.includes(mapping.nickname)) {
    throw createBadRequestError(`找不到昵称列: ${mapping.nickname}`);
  }
  if (!headers.includes(mapping.platform)) {
    throw createBadRequestError(`找不到平台列: ${mapping.platform}`);
  }
  if (!headers.includes(mapping.platformId)) {
    throw createBadRequestError(`找不到平台账号ID列: ${mapping.platformId}`);
  }

  const preview: ImportPreviewRow<InfluencerPreviewData>[] = [];
  let validRows = 0;
  let errorRows = 0;
  let duplicateRows = 0;
...
): { data: SamplePreviewData | null; errors: string[] } {
  const errors: string[] = [];
...
export async function previewSampleImport(
  ...
  if (!headers.includes(mapping.sku)) {
    throw createBadRequestError(`找不到SKU列: ${mapping.sku}`);
  }
  if (!headers.includes(mapping.name)) {
    throw createBadRequestError(`找不到名称列: ${mapping.name}`);
  }
  if (!headers.includes(mapping.unitCost)) {
    throw createBadRequestError(`找不到单件成本列: ${mapping.unitCost}`);
  }
  if (!headers.includes(mapping.retailPrice)) {
    throw createBadRequestError(`找不到建议零售价列: ${mapping.retailPrice}`);
  }

  const preview: ImportPreviewRow<SamplePreviewData>[] = [];
  let validRows = 0;
  let errorRows = 0;
  let duplicateRows = 0;
 ...
 */
export function suggestInfluencerMapping(headers: string[]): Partial<InfluencerFieldMapping> {
  const mapping: Partial<InfluencerFieldMapping> = {};
 ...
 */
export function suggestSampleMapping(headers: string[]): Partial<SampleFieldMapping> {
  const mapping: Partial<SampleFieldMapping> = {};
