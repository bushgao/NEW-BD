...
// 回本状态名称映射
export const PROFIT_STATUS_NAMES: Record<ProfitStatus, string> = {
  LOSS: '未回本',
  BREAK_EVEN: '刚回本',
  PROFIT: '已回本',
  HIGH_PROFIT: '爆赚',
 ...
 */
export function calculateProfitStatus(roi: number): ProfitStatus {
  if (roi < 1) return 'LOSS';
  if (roi === 1) return 'BREAK_EVEN';
  if (roi < 3) return 'PROFIT';
  return 'HIGH_PROFIT';
}
 ...
 * 如果总成本为0，返回0避免除零错误
 */
export function calculateRoi(salesGmv: number, totalCost: number): number {
...
export async function createResult(data: CreateResultInput, brandId: string) {
  ...
  if (collaboration.result) {
    throw createBadRequestError('该合作已有结果记录，请使用更新接口');
  }

  // 计算关联寄样的总成本
  const totalSampleCost = collaboration.dispatches.reduce(
...
export async function getResultById(id: string, brandId: string) {
  ...
  if (!result) {
    throw createNotFoundError('合作结果不存在');
  }
...
export async function updateResult(
  ...
  if (data.salesQuantity !== undefined) updateData.salesQuantity = data.salesQuantity;
  if (data.salesGmv !== undefined) updateData.salesGmv = data.salesGmv;
  if (data.commissionRate !== undefined) updateData.commissionRate = data.commissionRate;
  if (data.pitFee !== undefined) updateData.pitFee = data.pitFee;
  if (data.actualCommission !== undefined) updateData.actualCommission = data.actualCommission;
  if (data.willRepeat !== undefined) updateData.willRepeat = data.willRepeat;
  if (data.notes !== undefined) updateData.notes = data.notes?.trim() || null;

  // 如果更新了影响成本或GMV的字段，重新计算ROI
  const needRecalculate =
    data.salesGmv !== undefined ||
  ...
  const result = await prisma.collaborationResult.update({
    where: { id },
    data: updateData,
    include: {
      collaboration: {
...
export async function listResults(
  ...
  const total = await prisma.collaborationResult.count({ where });

  const data = await prisma.collaborationResult.findMany({
    where,
...
export async function getResultStats(brandId: string, dateRange?: { startDate: Date; endDate: Date }) {
  ...
  // 获取各状态数量
  const statusCounts = await prisma.collaborationResult.groupBy({
    by: ['profitStatus'],
    where,
    _count: { id: true },
  });
  ...
  // 获取汇总数据
  const aggregation = await prisma.collaborationResult.aggregate({
    where,
    _sum: {
      salesGmv: true,
